...
**Created**: 2026-01-28
**Status**: Draft
**Input**: User description: "Update doc-serve-skill to know about the new multi-instance architecture features including init, start, stop, list commands, auto-port discovery, and runtime.json reading for sharing instances among coding agents"

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Project Initialization via Skill (Priority: P1)

As a Claude Code user, I want the doc-serve skill to automatically initialize a project for doc-serve when I first use it, so that I don't need to manually run CLI commands before I can search my documentation.

**Why this priority**: This is the foundational capability. Without automatic initialization, users must know about and run `doc-svr-ctl init` manually, which defeats the purpose of having a skill.

**Independent Test**: Invoke the skill in a project without existing doc-serve configuration, verify it creates the `.claude/doc-serve/` directory and `config.json`, and confirm the skill reports successful initialization.

**Acceptance Scenarios**:

1. **Given** a project without `.claude/doc-serve/` directory, **When** the user invokes the doc-serve skill, **Then** the skill initializes the project by running `doc-svr-ctl init` and reports the result.
...
As a Claude Code user, I want the skill to automatically discover running doc-serve instances or start one if needed, so that I can immediately search my documentation without manual server management.
...
1. **Given** a running doc-serve instance for the project, **When** the skill is invoked, **Then** it discovers the instance via `runtime.json` and connects to it.
2. **Given** no running doc-serve instance, **When** the skill is invoked, **Then** it starts a server with `doc-svr-ctl start --daemon`, waits for readiness, and then proceeds with the operation.
...
As a Claude Code user, I want the skill to report the status of doc-serve instances including port, mode, and indexing state, so that I can understand the current state of my search infrastructure.
...
1. **Given** a running doc-serve instance, **When** the user asks for status, **Then** the skill reports: port, mode, instance ID, project ID, document count, and indexing status.
...
3. **Given** multiple instances across different projects, **When** the user asks for a list, **Then** the skill runs `doc-svr-ctl list` and presents all instances with their project roots.

---

### User Story 4 - Instance Lifecycle Management (Priority: P2)

As a Claude Code user, I want the skill to stop doc-serve instances when requested, so that I can free resources when I'm done working on a project.
...
1. **Given** a running doc-serve instance, **When** the user asks to stop the server, **Then** the skill runs `doc-svr-ctl stop` and confirms shutdown.
...
As a team of Claude Code agents working on the same project, I want to share a single doc-serve instance, so that multiple agents can query the same indexed documentation without resource duplication.
...
**Independent Test**: Start a doc-serve instance from one agent context, invoke the skill from a different agent context in the same project, verify both agents connect to the same instance by checking the instance ID.

**Acceptance Scenarios**:

1. **Given** a doc-serve instance started by Agent A, **When** Agent B invokes the skill in the same project, **Then** Agent B discovers and uses the existing instance.
...
- **FR-001**: Skill MUST automatically initialize projects by creating `.claude/doc-serve/` directory and `config.json` when invoked in an uninitialized project.
- **FR-002**: Skill MUST discover running doc-serve instances by reading `runtime.json` from the project's state directory (`.claude/doc-serve/runtime.json`).
- **FR-003**: Skill MUST start a doc-serve server automatically when no running instance is discovered, using `doc-svr-ctl start --daemon` with auto-port allocation.
...
- **FR-007**: Skill MUST support stopping servers via `doc-svr-ctl stop` when requested by the user.
- **FR-008**: Skill MUST support listing all running doc-serve instances via `doc-svr-ctl list`.
...
- **Runtime State**: JSON file at `.claude/doc-serve/runtime.json` containing mode, port, base_url, pid, instance_id, project_id, and started_at timestamp.
- **Project Configuration**: JSON file at `.claude/doc-serve/config.json` containing optional settings like preferred_mode and custom paths.
...
- **SC-001**: Users can invoke the doc-serve skill and perform searches without any manual CLI commands for server management.
...
- The CLI tool (`doc-svr-ctl`) is installed and available in PATH.
